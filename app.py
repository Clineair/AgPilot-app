...
# ────────────────────────────────────────────────
# Density Altitude Calculation (from Enstrom 480 POH)
# ────────────────────────────────────────────────
...
def compute_weight_balance(fuel_gal, hopper_gal, pilot_weight_lbs, aircraft):
    data = AIRCRAFT_DATA[aircraft]
    empty_weight = st.session_state.get('custom_empty_weight', data["base_empty_weight_lbs"])
...
if st.session_state.fleet:
    ...
    if selected_nickname != "— Select a saved aircraft —":
        ...
        st.session_state.custom_empty_weight = entry["custom_empty"]
        st.success(f"Loaded **{selected_nickname}** ({entry['aircraft']}) – Empty: {entry['custom_empty']} lb")
else:
